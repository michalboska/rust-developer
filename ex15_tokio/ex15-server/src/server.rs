...
impl<T: Serialize + DeserializeOwned + Send + 'static> Server<T> {
    pub fn new(socket_addr: SocketAddr) -> Result<Server<T>, ServerError> {
        info!("Listening on {}", socket_addr);
        Ok(Server {
            listener: TcpListener::bind(socket_addr).map_err(|_| AddressInUseError(socket_addr))?,
            clients: Arc::new(Mutex::new(HashMap::new())),
        })
    }

    pub fn listen_blocking(&mut self) -> Result<(), ServerError> {
